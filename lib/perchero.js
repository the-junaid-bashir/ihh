...
import { Terminal, Package, GitBranch, ShieldCheck, ArrowRight } from "lucide-react";

const PercolatorMarketHero = () => {
  return (
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="space-y-8">
            ...
            <p className="max-w-md text-gray-400 leading-relaxed border-l-2 border-white/10 pl-6">
              PPM turns raw Percolator commands into versioned, installable packages.  
              Structured configs. Deterministic presets. Forkable environments.  
              Same config. Same output. Every machine.
            </p>

            {/* Feature Grid */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4">
              {[
                { label: "Publish", val: "ipm op publishmarkettemplate <name>" },
                { label: "Install", val: "ipm op installmarkettemplate <name>" },
                { label: "Deterministic", val: "Same command outputs" },
                { label: "Profiles", val: "devnet / mainnet / local" },
                { label: "Shareable", val: "Forkable, versioned setups" },
              ].map((item, i) => (
            ...
            <div className="relative bg-[#0A0A0A] border border-white/10 rounded-sm overflow-hidden shadow-2xl">
              ...
              <div className="bg-[#1A1A1A] px-4 py-3 border-b border-white/10 flex items-center justify-between">
                ...
                <span className="text-[10px] text-gray-500 tracking-[0.2em] uppercase flex items-center gap-2">
                  <Terminal size={12} /> ppm_market.pkg
                </span>
              </div>
...
export default PercolatorMarketHero;
