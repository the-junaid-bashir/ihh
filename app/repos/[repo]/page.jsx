...
export default function FolderDetail({ params }) {
  ...
  const decodedFolder = decodeURIComponent(repo);
...
useEffect(() => {
  ...
  async function checkhosting() {
    try {
      const response = await fetch(`/api/hosted?uid=${vjwt.sub}&folder=${repo}`, {
        method: "GET",
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {files.length > 0 ? (
            files.map(file => (
              <Link
                key={file.cid}
                href={`/folders/${encodeURIComponent(repo)}/${file.cid}`}
