...
import { useOpenData } from "@/lib/useOpenData";
import { 
...
export default function FilePage({ params }) {
  ...
  const { uploads, loading } = useOpenData();
      ...
      <main className="max-w-6xl mx-auto px-6 py-10 pb-32">
        ...
        <div className="space-y-6">
          <div className="flex items-center gap-4 border-b border-white/5">
            <button onClick={() => setActiveTab("ai")} className={`pb-4 text-xs font-bold uppercase tracking-widest flex items-center gap-2 transition-all ${activeTab === 'ai' ? 'text-[#A359FF] border-b-2 border-[#A359FF]' : 'text-zinc-500'}`}>
              <Sparkles className="w-3.5 h-3.5" /> AI Review
            </button>
            <button onClick={() => setActiveTab("static")} className={`pb-4 text-xs font-bold uppercase tracking-widest flex items-center gap-2 transition-all ${activeTab === 'static' ? 'text-red-400 border-b-2 border-red-400' : 'text-zinc-500'}`}>
              <ShieldCheck className="w-3.5 h-3.5" /> Static Analysis
            </button>
          </div>
